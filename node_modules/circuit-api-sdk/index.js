...
const user_agent    = 'Mozilla/5.0 (Linux; 1.0.0) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/1.0';
...
const getGetConversationFeedMsg = function(_self, resolve, reject, convid, minTotalItems, timestamp, commentsPerThread) {
    let nextId = _self.nextReqID();
    _self.resolver[nextId] = resolve;
    _self.rejecter[nextId] = reject;
    let r = '{"msgType":"REQUEST","request":{"requestId":' + nextId + ',"type":"CONVERSATION","conversation":{"type":"GET_CONVERSATION_FEED","getConversationFeed":{"convId":"' + convid + '","minTotalItems":' + minTotalItems + ',"timestamp":' + timestamp + ',"commentsPerThread":' + commentsPerThread + '}}}}';
    return r;
};

const getGetThreadCommentsMsg = function(_self, resolve, reject, convid, parentItemId, timestamp, direction, totalComments) {
    let nextId = _self.nextReqID();
    _self.resolver[nextId] = resolve;
    _self.rejecter[nextId] = reject;
    let r = '{"msgType":"REQUEST","request":{"requestId":' + nextId + ',"type":"CONVERSATION","conversation":{"type":"GET_THREAD_COMMENTS","getThreadComments":{"convId":"' + convid + '","threadId":"' + parentItemId + '","timestamp":' + timestamp + ',"timestampFilter":"CREATION","direction":"' + direction + '","totalComments":' + totalComments + '}}}}';
    return r;
};
...
const getRemoveParticipantsMsg = function(_self, resolve, reject, convId, participants) {
    let nextId = _self.nextReqID();
    _self.resolver[nextId] = resolve;
    _self.rejecter[nextId] = reject;
    let r = '{"msgType":"REQUEST","request":{"requestId":' + nextId + ',"type":"CONVERSATION","conversation":{"type":"REMOVE_PARTICIPANT","removeParticipant":{"convId":"' + convId + '","hardRemove":"true","userId":' + participants + '}}}}';
    return r;
};
                    ...
                    case 'CONVERSATION':
                        switch (data.response.conversation.type) {
                            ...
                            case 'GET_MARKED_CONVERSATIONS_LIST':
                                ...
                                break;
                            case 'GET_CONVERSATION_FEED':
                                return resolve(data.response.conversation.getConversationFeedResult);
                                break;
                            case 'GET_THREAD_COMMENTS':
                                return resolve(data.response.conversation.getThreadCommentsResult);
                                break;
                            default:
...
Circuit.prototype.getConversationFeed = function(convId, data = {}) {
    const _self = this;
    return new Promise((resolve, reject) => {
        _self.prepwssend(getGetConversationFeedMsg(_self, resolve, reject, convId,
                                                (data.minTotalItems ? data.minTotalItems : 25),
                                                (data.date ? data.date : new Date().getTime()),
                                                (data.commentsPerThread ? data.commentsPerThread : 3)
                                                ));
    });
};

Circuit.prototype.getThreadComments = function(convId, parentItemId, data = {}) {
    const _self = this;
    return new Promise((resolve, reject) => {
        _self.prepwssend(getGetThreadCommentsMsg(_self, resolve, reject, convId, parentItemId,
                                                (data.date ? data.date : new Date().getTime()),
                                                (data.direction ? data.direction : 'AFTER'),
                                                (data.totalComments ? data.totalComments : 10)
                                                ));
    });
};
...
Circuit.prototype.removeParticipants = function(convId, participants) {
    const _self = this;
    return new Promise((resolve, reject) => {
        _self.prepwssend(getRemoveParticipantsMsg(_self, resolve, reject, convId, participants));
    });
};
